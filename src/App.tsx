...
import { ArrowLeft, ChevronRight, Copy, Crown, Dice5, Flame, Gamepad2, Home, Menu, Share2, Shield, Trophy, UserRound, Zap } from 'lucide-react';
...
function Avatar({ label = devProfile.handle, large = false }: { label?: string; large?: boolean }) {
  return <div className={`avatar ${large ? 'avatar-large' : ''}`} aria-label={`${label} avatar`}><span>{label.replace(/[^a-z0-9]/gi, '').slice(0, 2).toUpperCase()}</span></div>;
}

function Badge({ children, tone = 'violet' }: { children: React.ReactNode; tone?: 'violet' | 'orange' | 'green' }) {
...
function App() {
  ...
  const [challengeCode] = useState('A7K9X');
  ...
  const copyCode = async () => {
    try { await navigator.clipboard?.writeText(challengeCode); setCopied(true); window.setTimeout(() => setCopied(false), 1400); } catch { setCopied(false); }
  };
  ...
  return <div className="app-shell">
    ...
    <main className="content" key={screen}>
      ...
      {screen === 'match' && <MatchView go={go} mode={mode} setMode={setMode} />}
      {screen === 'challenge' && <ChallengeView go={go} code={challengeCode} copied={copied} copyCode={copyCode} />}
      {screen === 'ludo' && <LudoView go={go} turn={turn} setTurn={setTurn} />}
...
function HomeView({ go }: { go: (s: Screen) => void }) {
  return <>
    ...
    <button className="feature-card" onClick={() => go('match')}><div className="board-mini">{Array.from({ length: 16 }).map((_, i) => <span key={i} className={i % 5 === 0 ? 'hot' : ''} />)}</div><div className="feature-copy"><Badge>AVAILABLE</Badge><h3>The classic.<br /><em>Reimagined.</em></h3><p>Strategy, luck, and one more turn.</p></div><ChevronRight className="feature-arrow" /></button>
    ...
    <div className="leader-preview">{devLeaderboard.slice(0, 3).map((entry) => <div className={`leader-line ${entry.isCurrentUser ? 'current' : ''}`} key={entry.rank}><span className="leader-rank">{entry.rank === 1 ? '01' : entry.rank === 2 ? '02' : '03'}</span><Avatar label={entry.handle} /><span className="leader-name">{entry.handle}</span><strong>{entry.rating.toLocaleString()}</strong></div>)}</div>
  </>;
}
