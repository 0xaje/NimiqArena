...
import { applyCommand, createLudoSnapshot, type LudoSnapshot } from "@shared/game/ludo-engine";

const roll = (snapshot: LudoSnapshot, nonce: string, value: number) => applyCommand(snapshot, { kind: "roll", matchId: snapshot.matchId, playerId: snapshot.currentPlayer, expectedVersion: snapshot.version, nonce }, () => value);
const move = (snapshot: LudoSnapshot, nonce: string, pieceIndex = 0) => applyCommand(snapshot, { kind: "move", matchId: snapshot.matchId, playerId: snapshot.currentPlayer, expectedVersion: snapshot.version, nonce, pieceIndex }, () => 1);
...
describe("ludo engine", () => {
  it("creates a deterministic initial snapshot", () => {
    ...
    expect(snapshot.players[0].pieces.every(piece => piece.position === -1)).toBe(true);
  });
  ...
  it("rejects stale versions and duplicate nonces", () => {
    ...
    expect(first.ok).toBe(true);
    if (!first.ok) return;
    expect(roll(first.snapshot, "same-nonce", 2)).toMatchObject({ ok: false, code: "DUPLICATE_NONCE" });
    expect(applyCommand(first.snapshot, { kind: "roll", matchId: first.snapshot.matchId, playerId: 0, expectedVersion: 0, nonce: "new-nonce" }, () => 2)).toMatchObject({ ok: false, code: "STALE_VERSION" });
  });
  ...
  it("captures an opponent on a non-safe track square", () => {
    ...
    if (result.ok) {
      ...
      expect(result.event).toMatchObject({ type: "moved", capturedPiece: { playerId: 1, pieceIndex: 0 } });
    }
  });
  ...
  it("returns a winner when all pieces reach home", () => {
    const snapshot = createLudoSnapshot("match-1");
    snapshot.dice = 1;
    snapshot.players[0].pieces = snapshot.players[0].pieces.map((piece, index) => ({ position: index === 0 ? 56 : 57 }));
